...
export default function CommentsSection({ animeId, isAuthed, userId }: { animeId: number; isAuthed: boolean; userId: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    if (!data || data.length === 0) { setComments([]); setVotes([]); return; }
  ...
  async function handlePost(parentId: number | null) {
    ...
    setText(""); setReplyText(""); setReplyTo(null); setError("");
    await load();
  }
  ...
  async function handleVote(commentId: number, v: number) {
    ...
    if (existing?.vote === v) {
      await supabase.from("comment_votes").delete().eq("user_id", userId).eq("comment_id", commentId);
    } else if (existing) {
      await supabase.from("comment_votes").update({ vote: v }).eq("user_id", userId).eq("comment_id", commentId);
    } else {
      await supabase.from("comment_votes").insert({ user_id: userId, comment_id: commentId, vote: v });
    }
    await load();
  }
  ...
  function voteCount(id: number, dir: number) {
    return votes.filter((x) => x.comment_id === id && x.vote === dir).length;
  }
  function myVote(id: number) {
    return votes.find((x) => x.comment_id === id && x.user_id === userId)?.vote || 0;
  }
        ...
        {roots.map((c) => (
          <div key={c.id} className="p-3 bg-[#121214] border border-[#222226] rounded-lg">
            <div className="flex items-center gap-2 mb-1.5">
              <Link href={`/profile/${c.user_id}`} className="w-7 h-7 rounded-full bg-gradient-to-tr from-sky-400 to-blue-600 flex items-center justify-center text-white text-[10px] font-black overflow-hidden shrink-0 relative">
              ...
              {(c.user_id === userId || isAdmin) && (
                <button onClick={() => handleDelete(c.id, c.user_id)} className="text-[10px] text-red-400 hover:text-red-300"><i className="fa-solid fa-trash-can"></i></button>
              )}
            </div>
            <p className="text-xs text-gray-300 whitespace-pre-wrap break-words">{c.text}</p>
            <div className="flex items-center gap-2 mt-2">
              <button onClick={() => handleVote(c.id, 1)} className={`flex items-center gap-1 text-[11px] px-2 py-1 rounded border transition-all ${myVote(c.id) === 1 ? "bg-green-500/20 text-green-400 border-green-500/30" : "text-gray-500 border-[#222226] hover:text-green-400"}`}>
                <i className="fa-solid fa-thumbs-up text-[10px]"></i> {voteCount(c.id, 1) || ""}
              </button>
              <button onClick={() => handleVote(c.id, -1)} className={`flex items-center gap-1 text-[11px] px-2 py-1 rounded border transition-all ${myVote(c.id) === -1 ? "bg-red-500/20 text-red-400 border-red-500/30" : "text-gray-500 border-[#222226] hover:text-red-400"}`}>
                <i className="fa-solid fa-thumbs-down text-[10px]"></i> {voteCount(c.id, -1) || ""}
              </button>
              {isAuthed && (
                <button onClick={() => setReplyTo(replyTo === c.id ? null : c.id)} className="text-[11px] text-sky-400 hover:text-sky-300 ml-2"><i className="fa-solid fa-reply mr-1"></i>Ответить</button>
              )}
            </div>
            {replyTo === c.id && (
              ...
              {children(c.id).map((ch) => (
                <div key={ch.id} className="ml-4 pl-3 border-l border-[#222226] p-2 bg-[#1a1a1e] rounded">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {(ch.user_id === userId || isAdmin) && (
                      <button onClick={() => handleDelete(ch.id, ch.user_id)} className="text-[10px] text-red-400"><i className="fa-solid fa-trash-can"></i></button>
                    )}
                  </div>
                  <p className="text-xs text-gray-300 whitespace-pre-wrap break-words">{ch.text}</p>
                  <div className="flex items-center gap-2 mt-1.5">
                    <button onClick={() => handleVote(ch.id, 1)} className={`flex items-center gap-1 text-[11px] px-2 py-0.5 rounded border ${myVote(ch.id) === 1 ? "bg-green-500/20 text-green-400 border-green-500/30" : "text-gray-500 border-[#222226]"}`}>
                      <i className="fa-solid fa-thumbs-up text-[10px]"></i> {voteCount(ch.id, 1) || ""}
                    </button>
                    <button onClick={() => handleVote(ch.id, -1)} className={`flex items-center gap-1 text-[11px] px-2 py-0.5 rounded border ${myVote(ch.id) === -1 ? "bg-red-500/20 text-red-400 border-red-500/30" : "text-gray-500 border-[#222226]"}`}>
                      <i className="fa-solid fa-thumbs-down text-[10px]"></i> {voteCount(ch.id, -1) || ""}
                    </button>
                  </div>
                </div>
              ))}
